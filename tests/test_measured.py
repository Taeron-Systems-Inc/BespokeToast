...
def test_a_profile_with_no_liquidus_is_not_judged_on_time_above_it():
    ...
    "time above liquidus FAILED 1033 s" on an otherwise clean run, and a
    report that cries wolf is one people stop reading."""
    from oven.metrics import RunMetrics, Limits as MLimits
    cold = RunMetrics(0.0)
    for t in range(0, 200, 5):
        cold.add(float(t), 100.0)
    assert "time above liquidus" not in [c[0] for c in cold.check(MLimits())]
